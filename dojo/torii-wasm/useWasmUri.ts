...
import { fetchWasm } from './fetchWasm'
    ...
    () =>
      void (async () => {
        try {
          setState({
            loading: false,
            result: await WebAssembly.instantiate<Exports>(await fetchWasm(uri), importObject),
          })
        } catch (cause) {
          setState({
            loading: false,
            error: new Error(`Failed to instantiate "${uri}".`, cause as ErrorOptions),
          })
        }
      })(),
    [uri, importObject]
